# COMS Pytest - Complete Framework Documentation

**Version**: 1.0.0  
**Created**: March 17, 2026  
**Project**: COMS Pytest - API Testing Framework
...
COMS Pytest is a modern, comprehensive API testing framework built with pytest, offering:
...
- ✅ **Beautiful Reporting** - Allure HTML reports with trends
...
COMS Pytest is an enterprise-grade API testing framework that simplifies test creation and maintenance with automatic authentication, token management, and comprehensive reporting.
...
│   │   └── __init__.py
│   ├── wrappers/
│   │   ├── organization_wrapper.py # Response wrappers
│   │   └── __init__.py
│   └── __init__.py
│
...
│   │   └── __init__.py
│   ├── organisation/
│   │   ├── test_organization.py    # Organization CRUD
...
response = client.get("/api/v1/organizations", params={"page": 1})

# POST
response = client.post("/api/v1/organizations", json={"name": "Org"})

# PUT
response = client.put("/api/v1/organizations/123", json={"name": "Updated"})

# PATCH
response = client.patch("/api/v1/organizations/123", json={"status": "active"})
...
| `login_endpoint` | Yes | Login endpoint | `/api/v1/user/login` |
| `refresh_endpoint` | Yes | Token refresh endpoint | `/api/v1/user/refreshToken` |
| `credentials.email` | Yes | User email | `user@example.com` |
| `credentials.password` | Yes | User password | `password123` |
...
@allure.story("Create")
@allure.title("Create new organization")
...
```python
def test_with_steps(api_client):
...
response = client.post("/api/v1/user/login", json=creds, auth_required=False)
```
...
def test_with_payload(api_client, sample_payload):
    response = api_client.post("/api/v1/organizations", json=sample_payload)
    assert response.status_code == 200
```
...
def created_organization(api_client):
    """Create org and cleanup after test."""
    # Setup
    response = api_client.post("/api/v1/organizations", json={"name": "Test"})
    org_id = response.json()["responseObject"]["organizationId"]
...
# Use in test
def test_with_created_org(created_organization):
...
```python
@pytest.fixture(params=["Org1", "Org2", "Org3"])
def org_names(request):
    """Parametrized fixture."""
    return request.param

# Test runs once for each parameter
def test_create_orgs(api_client, org_names):
    response = api_client.post(
        "/api/v1/organizations",
        json={"name": org_names}
    )
...
def test_create_org_1(api_client):
    response = api_client.post("/api/v1/organizations", json={"name": "Org1"})
    assert response.status_code == 200

def test_create_org_2(api_client):
    response = api_client.post("/api/v1/organizations", json={"name": "Org2"})
    assert response.status_code == 200
...
def test_create_org_with_valid_data():
    pass
...
def test_create_orgs(api_client, org_name):
    response = api_client.post("/api/v1/organizations", json={"name": org_name})
    assert response.status_code == 200
...
assert response.status_code == 200, \
    f"Create failed: {response.json().get('message')}"
```
...
│   ├── test_org_validation.py
│   ├── test_org_pagination.py
│   └── __init__.py
└── __init__.py
```
...
```bash
# Check credentials
email=$(grep email config/dev.yaml)
password=$(grep password config/dev.yaml)
...
|----------|--------|---------|---------|----------|
| `/api/v1/user/login` | POST | User login | `{userName, password}` | `{accessToken, refreshToken}` |
| `/api/v1/user/refreshToken` | POST | Refresh access token | `{refreshToken}` | `{accessToken, refreshToken}` |
...
response = api_client.get("/api/v1/resource")  # Refreshes if needed
```
...
```python
@pytest.mark.parametrize("org_name", ["Org1", "Org2"])
def test_create_orgs(api_client, org_name):
    response = api_client.post(
        "/api/v1/organizations",
        json={"name": org_name}
...
COMS Pytest provides a comprehensive framework for API testing with:
...
For more information, refer to individual documentation files in the `docs/` directory.
